...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Привет! 👋\nЧтобы найти фильм, нажмите кнопку ниже:",
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    films = load_films()
    if code in films:
        await update.message.reply_text("❌ Код уже существует, выберите другой")
        return
    title = " ".join(args[1:])
...
async def edit_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if code not in films:
        ...
        return
    films[code]["title"] = " ".join(args[1:])
    save_films(films)
    await update.message.reply_text(f"Название фильма {code} изменено ✅")
...
async def edit_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not code:
        ...
        await update.message.reply_text(f"Теперь отправьте новый видеофайл для фильма {code}")
        return
    films = load_films()
    if code not in films:
        await update.message.reply_text("❌ Фильм с таким кодом не найден")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    waiting = context.user_data.get("waiting_code")
    if txt == "🔍 Поиск по коду" or waiting is None:
        ...
        return
    if not txt.isdigit():
        await update.message.reply_text("❌ Допускаются только цифры от 3 до 5")
        return
    if not (3 <= len(txt) <= 5):
        await update.message.reply_text("❌ Код должен быть от 3 до 5 цифр")
        return
...
async def send_film_by_code(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    try:
        ...
        context.user_data["waiting_code"] = None
    except Exception:
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.video:
        ...
        return
    films[code] = {"title": films.get(code, {}).get("title", context.user_data.get("add_title")), "file_id": file_id}
    save_films(films)
    if "add_code" in context.user_data:
        await update.message.reply_text(f"Фильм '{films[code]['title']}' с кодом {code} добавлен ✅")
        context.user_data.pop("add_code", None)
        context.user_data.pop("add_title", None)
    elif "edit_code" in context.user_data:
