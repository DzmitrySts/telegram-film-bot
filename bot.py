...
def get_inline_search_button():
    return InlineKeyboardMarkup.from_button(
        InlineKeyboardButton("🔍 Поиск по коду", callback_data="search")
    )
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    films = load_films()
    if code in films:
        await update.message.reply_text("⚠️ Фильм с таким кодом уже существует.")
        return
...
async def edit_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2:
        ...
        return
    code, new_title = args[0], " ".join(args[1:])
    films = load_films()
...
async def edit_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Теперь отправьте новое видео для кода {args[0]}")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not txt.isdigit():
        await update.message.reply_text("❌ Код должен состоять только из цифр (3–5).")
        return
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"Видео для фильма {code} обновлено ✅" if "edit_code" in context.user_data else f"Фильм '{title}' с кодом {code} добавлен ✅"
...
async def main():
    ...
    # Только команда поиска в меню
    menu_commands = [BotCommand("start", "🔍 Поиск по коду")]
