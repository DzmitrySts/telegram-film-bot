...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привет! 👋\nНажмите кнопку ниже, чтобы начать поиск фильма по коду.",
...
async def list_films(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not films:
        ...
        return
    sorted_items = sorted(films.items(), key=lambda x: x[0])
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    films = load_films()
    if code in films:
        await update.message.reply_text("❌ Код уже существует. Используйте другой.")
        return
    title = " ".join(args[1:])
...
async def edit_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("Использование: /editm <код> (отправьте видео после команды)")
        return
    code = args[0]
    films = load_films()
    if code not in films:
...
async def send_film_by_code(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    try:
        if file_id:
            await update.message.reply_video(video=file_id, caption=f"{caption}\n\nЧтобы найти другой фильм, нажмите снова кнопку «🔍 Поиск по коду»",
                                           reply_markup=InlineKeyboardMarkup(
                                               [[InlineKeyboardButton("🔍 Поиск по коду", callback_data="search_code")]]
                                           ))
        elif url:
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not code or (not title and "edit_code" not in context.user_data):
        return
    ...
    films = load_films()
    films[code] = {"title": title or films[code]["title"], "file_id": file_id}
    save_films(films)
    await update.message.reply_text(f"Фильм с кодом {code} обновлен ✅")
